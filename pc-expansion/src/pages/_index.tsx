...
export default function Index(){
 ...
 function manageBeast(action:BeastCommand):boolean {
  if(syncBlocked.current){setNotice('Reabra o jogo em uma única aba antes de continuar.');return false;}
  const base=settleFlow(true);
  try{
   const result=BeastSanctuary.command(base,action);const root=result.save;
   v5Ref.current=root;latestSave.current=ArenaCharacterAdapter.toLegacy(root);
   setBeasts(root.beasts);setCultivation(root.cultivation);setSave(latestSave.current);
   try{v5Ref.current=ArenaSaveV5.persist(root);lastFlush.current=Date.now();setStorageWarning('');}
   catch{setStorageWarning('Santuário mantido nesta sessão, mas o armazenamento falhou. Exporte um backup antes de fechar.');}
