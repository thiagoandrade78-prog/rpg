...
type Props={root:R.SaveV5;onBack:()=>void;onCultivation:()=>void;onCommand:(action:BeastCommand)=>boolean;storageWarning:string;};
export const ArenaBeasts=({root,onBack,onCultivation,onCommand,storageWarning}:Props)=>{
 ...
 return <section className={styles.root} data-beast-stage="3" data-reduced-motion={reduced?'true':'false'}>
  ...
  {tab==='sanctuary'&&<div role="tabpanel" id="beast-panel-sanctuary" aria-labelledby="beast-tab-sanctuary">
   {!adopted&&<>
    <div className={styles.welcome}><div><span>I · UM NOVO COMPANHEIRO</span><h2>Além do aço,<br/>um vínculo.</h2><p>Lyra cuida de três feras resgatadas. Após despertar seu núcleo, você pode acolher <b>uma delas, gratuitamente e uma única vez</b>. Cada candidata já tem seu potencial e sua linhagem definidos.</p><small>Não é captura selvagem nem assimilação. As próximas etapas acrescentam esses sistemas sem substituir sua coleção.</small>{!awake&&<Button className={styles.goldButton} onClick={onCultivation}><Sparkles size={17}/> DESPERTAR PARA ACOLHER</Button>}</div><BeastPortrait species="pyrofang" reduced={reduced}/></div>
    {confirmation}
