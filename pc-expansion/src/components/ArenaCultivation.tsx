...
export const ArenaCultivation=({save,model,onAwaken,onBack,onBackup,storageWarning,onCommand,offlineReport,onDismissReport,clockWarning}:Props)=>{
 ...
 return <section className={styles.root} data-expansion-stage="2" data-reduced-motion={save.reducedMotion?'true':'false'} data-awakened={awakened?'true':'false'}>
  ...
  {tab==='roadmap'&&<div role="tabpanel" id="cult-panel-roadmap" aria-labelledby="cult-tab-roadmap" className={styles.roadmap}>
   <div className={styles.roadmapIntro}><span className={styles.eyebrow}>CRÔNICAS DA EXPANSÃO</span><h2>Da centelha ao Avatar.</h2><p>Dez entregas sobre o jogo ilustrado. As etapas 1 e 2 estão disponíveis nesta versão.</p></div>
   {ExpansionRoadmap.map(stage=><article key={stage.id} className={stage.id<=2?styles.currentStage:styles.futureStage}><span>{String(stage.id).padStart(2,'0')}</span><div><h3>{stage.title}</h3><p>{stage.detail}</p></div>{stage.id<=2?<Check size={21}/>:<Lock size={17}/>}</article>)}
  </div>}
